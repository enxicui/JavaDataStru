...
public class DropTable implements SQLStatement {
	...
	@Override
	public SQLResult execute(Map<String, Table> table) throws SQLException {
		if (name.equals("mapsql.tables")) throw new SQLException("Table 'mapsql.tables' cannot be modified");
		Table tableName = table.get(name);
		if (tableName == null) throw new SQLException("Unknown table: " + name);
				
		table.get("mapsql.tables").delete(new Equals("table", name));
		table.remove (name);
